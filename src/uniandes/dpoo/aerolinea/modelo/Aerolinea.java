...
public class Aerolinea
{
    ...
    /** Un Mapa que tiene los vuelos realizados.
     * 
     */
    private Map<String, Vuelo> vuelosRealizados;
     ...
     */
    public void cargarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, InformacionInconsistenteException, AeropuertoDuplicadoException
    {
        // TODO implementar
     ...
     */
    public void salvarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, AeropuertoDuplicadoException
    {
        // TODO implementar
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for (Vuelo vuelo : vuelos) {
            if (vuelo.getAvion().getNombre().equals(nombreAvion) && vuelo.getFecha().equals(fecha)) {
                throw new Exception("El avión seleccionado ya está ocupado para otro vuelo en la misma fecha");
            }
        }
        ...
        if (ruta == null) {
            throw new Exception("La ruta especificada no existe en la aerolínea");
        }
        ...
        if (avion == null) {
            throw new Exception("El avión especificado no existe en la aerolínea");
        }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        // Buscamos al cliente con el identificador dado
        Cliente cliente = getCliente(identificadorCliente);
        if (cliente == null) {
            throw new Exception("El cliente especificado no existe.");
        }

        // Determinamos la temporada según la fecha del vuelo
        String[] partesFecha = fecha.split("-");
        int mes = Integer.parseInt(partesFecha[1]); 
        boolean temporadaAlta = (mes >= 6 && mes <= 8); 
        ...
        int tarifa = (int)calculadora.calcularTarifa(vuelo, cliente);
        ...
        int tiquetesVendidos = (vuelo.getTiquetes() != null) ? vuelo.getTiquetes().size() : 0;
        int asientosDisponibles = capacidadAvion - tiquetesVendidos;
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            vuelo.getTiquetes().add(tiquete); // Agregamos el tiquete al vuelo
        }
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
    	...
    	Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo != null) {
            vuelosRealizados.put(codigoRuta + fecha, vuelo);
        }
    }
